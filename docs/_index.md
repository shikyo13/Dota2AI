...
The generated ledger covers 336 tracked repository files and 212,499 lines outside `.git`, `.worktrees`, `node_modules`, and generated `docs/data`. Dependency extraction found 713 literal `require`, `dofile`, and TypeScript `import` edges.

## Canonical Vs Historical Docs

The canonical current docs are `AGENTS.md`, `campaign.md`, this index, `docs/tier1-quickref.md`, `docs/tier2-architecture.md`, `docs/tier2-gameplay-systems.md`, `docs/tier3-current-script-reference.md`, `docs/data/_index.md`, `docs/ARCHITECTURE.md`, `docs/PATCH_UPDATE_GUIDE.md`, and `docs/BOT_API_REFERENCE.md`.

Research docs under `docs/research/` are evidence snapshots. Read their top notes before treating local-fork status statements as current.

Docs under `docs/guides/` and `docs/plans/` are historical or meta documentation carried over for recovery context. They are not canonical current-script truth unless routed from this index.
